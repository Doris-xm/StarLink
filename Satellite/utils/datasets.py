...
class LoadImages:  # for inference
    def __init__(self, data, img_size=640):
        self.data = bytes(data)  # Convert data to bytes
        ...
        if self.video_flag[self.count]:
            ...
            if not ret_val:
                ...
                if self.count == self.nf:  # last video
                    ...
                    self.new_video(self.data)
                    ret_val, img0 = self.cap.read()

            self.frame += 1
            print('video %g/%g (%g/%g) %s: ' % (self.count + 1, self.nf, self.frame, self.nframes, 'Video'), end='')

        else:
            # Read image
            self.count += 1
            img0 = cv2.imdecode(np.frombuffer(self.data, np.uint8), cv2.IMREAD_COLOR)  # BGR
            assert img0 is not None, 'Invalid image data'
            print('image %g/%g %s: ' % (self.count, self.nf, 'Image'), end='')
    ...
    def new_video(self, data):
        self.frame = 0
        self.cap = cv2.VideoCapture(io.BytesIO(data))
